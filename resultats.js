...
class SchoolResultsSystem {
    constructor() {
        ...
        this.studentsData = {
            ...
            "bani": {
                ...
                comment_es: "¡Rahama brilla particularly en educación social donde obtuvo la puntuación máxima! Su compromiso en matemáticas merece ser apoyado."
            },
            "dahougou": {
                name: "DAHOUGOU Noham",
                password: "noham",
    ...
    init() {
        ...
        // La gestion du thème est maintenant dans theme.js
    ...
    setupEventListeners() {
        ...
        // const themeToggle = document.getElementById('themeToggle'); // Géré par theme.js
        const navItems = document.querySelectorAll('.nav-item[data-tab]'); // Plus spécifique
        ...
        // if(themeToggle) { // Géré par theme.js
    ...
    changeLanguage(lang) {
        ...
        if (resultsContainer && resultsContainer.style.display === 'block' && this.currentStudentId) {
            const student = this.studentsData[this.currentStudentId];
    ...
    // toggleTheme() { // Géré par theme.js
    // }

    // loadThemePreference() { // Géré par theme.js
    // }
    ...
    async handleLogin() {
        ...
        if (enteredPassword === student.password) {
            ...
            this.currentStudentId = selectedStudent;
            this.displayResults(student);
        ...
        return `
                        ...
                        <tr>
                            ${student.notes.map((note, index) => {
                                ...
                                return `<td class="note-cell ${noteClass}">${note}
                                            <div class="note-bar" style="width: ${note*5}%"></div>
                                        </td>`;
                            }).join('')}
                        </tr>
                    </tbody>
            ...
            <div class="smart-actions">
                ...
                <button class="btn" id="exportPdfBtn">
                    <i class="fas fa-file-pdf"></i> ${this.translateMessage('Télécharger PDF', 'Download PDF', 'Descargar PDF')}
                </button>
                <button class="btn" id="exportWordBtn">
                    <i class="fas fa-file-word"></i> ${this.translateMessage('Télécharger Word', 'Download Word', 'Descargar Word')}
                </button>
            </div>
        `;
    }
    ...
    setupResultsInteractions() {
        document.getElementById('printBulletinBtn').addEventListener('click', () => {
            this.hideBulletin();
            this.generatePrintableBulletin();
        });
    ...
    generatePrintableBulletin() {
        if (!this.currentStudentId || !this.currentStats) {
            this.showNotification(
                this.translateMessage('Aucun étudiant sélectionné', 'No student selected', 'Ningún estudiante seleccionado'),
                'error'
            );
            return;
        }
        ...
        bulletin.innerHTML = `
            <button class="close-btn no-print" onclick="schoolSystem.hideBulletin()">
            ...
            <div class="bulletin-actions no-print">
                ...
                <button class="btn no-print" onclick="schoolSystem.exportToPDF()">
                    <i class="fas fa-file-pdf"></i> ${this.translateMessage('Télécharger PDF', 'Download PDF', 'Descargar PDF')}
                </button>
                <button class="btn no-print" onclick="schoolSystem.exportToWord()">
                    <i class="fas fa-file-word"></i> ${this.translateMessage('Télécharger Word', 'Download Word', 'Descargar Word')}
                </button>
    ...
    exportToPDF() {
        ...
        const htmlContent = `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Bulletin Scolaire - ${student.name}</title>
                <style>
                    body { font-family: Arial, sans-serif; margin: 20px; }
                    .header { text-align: center; margin-bottom: 30px; }
                    .student-info { margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #2E86AB; }
                    .stats { display: flex; margin-bottom: 20px; }
                    .stat-card { flex: 1; background: #f8f9fa; padding: 15px; margin: 0 10px; border-radius: 8px; border-left: 4px solid #2E86AB; }
                    table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
                    th, td { border: 1px solid #ddd; padding: 12px; text-align: center; }
                    th { background-color: #2E86AB; color: white; }
                    .comment { background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #27AE60; }
                    .signatures { display: flex; justify-content: space-between; margin-top: 40px; }
                    .signature { text-align: center; width: 45%; }
                    .signature-line { border-top: 1px solid #000; width: 100%; margin: 10px 0; }
                </style>
            </head>
            <body>
                <div class="header">
                    <h1 style="color: #2E86AB; margin-bottom: 5px;">École "Les Bulles de Joie"</h1>
                    <h2 style="color: #FF7B54;">${this.translateMessage('Bulletin Scolaire - CE1 - Octobre 2025', 'School Report - CE1 - October 2025', 'Boletín Escolar - CE1 - Octubre 2025')}</h2>
                </div>
                
                <div class="student-info">
                    <h3 style="color: #2E86AB;">${this.translateMessage('Élève:', 'Student:', 'Estudiante:')} ${student.name}</h3>
                </div>
                
                <div class="stats">
                    <div class="stat-card">
                        <strong>${this.translateMessage('Moyenne Générale:', 'Overall Average:', 'Promedio General:')}</strong><br>
                        <span style="font-size: 1.4rem; font-weight: bold; color: #2E86AB;">${stats.average}/20</span>
                    </div>
                    <div class="stat-card">
                        <strong>${this.translateMessage('Matière Excellente:', 'Excellent Subject:', 'Materia Excelente:')}</strong><br>
                        <span style="font-size: 1.2rem; font-weight: bold; color: #FF7B54;">${stats.bestSubject}</span><br>
                        <small>${stats.bestNote}/20</small>
                    </div>
                </div>
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ${student.notes.map(note => `<td style="font-weight: bold;">${note}</td>`).join('')}
                        </tr>
                    </tbody>
                </table>
                
                <div class="comment">
                    <strong style="color: #2E86AB;">${this.translateMessage('Commentaire de l\'enseignant:', 'Teacher\'s comment:', 'Comentario del profesor:')}</strong><br>
                    <p style="margin-top: 10px; line-height: 1.6;">${this.getStudentComment(student)}</p>
                </div>
        ...
        this.showNotification(
            this.translateMessage('PDF généré avec succès!', 'PDF generated successfully!', '¡PDF generado con éxito!'),
            'success'
        );
    }
    ...
    exportToWord() {
        ...
        a.href = url;
        a.download = `Bulletin_${student.name.replace(/\s+/g, '_')}.doc`;
        ...
        this.showNotification(
            this.translateMessage('Document Word téléchargé!', 'Word document downloaded!', '¡Documento Word descargado!'),
            'success'
        );
    }
    ...
    animateResultsAppearance() {
        ...
        setTimeout(() => {
            document.querySelectorAll('.note-bar').forEach(bar => {
                // La transformation est maintenant gérée par la largeur dans le HTML
...
document.addEventListener('DOMContentLoaded', function() {
    // On vérifie si on est sur la page des résultats avant de lancer le gros script
    if (document.getElementById('studentSelect')) {
        schoolSystem = new SchoolResultsSystem();
        // Expose la fonction hideBulletin globalement pour le bouton "Fermer"
